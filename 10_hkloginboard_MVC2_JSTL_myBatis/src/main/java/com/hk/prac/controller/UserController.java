...
import com.hk.prac.daos.UserDao;
import com.hk.prac.dtos.UserDto;
...
public class UserController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(command.equals("/registForm.user")) {	//회원가입폼 이동
			...
			if(isS) {
				jsForward("회원가입 성공", "home.user", response);
			} else {
				jsForward("회원가입 실패", "registForm.user", response);
			}
			...
			if(ldto==null || ldto.getId()==null) {
				...
				} else if(ldto.getRole().equals("manager")) {
		...
		} else if(command.equals("/myinfo.user")) {	//나의 정보 조회
			String id=request.getParameter("id");
			UserDto dto=dao.getUserInfo(id);
			request.setAttribute("dto", dto);
			dispatch("userInfo.jsp", request, response);
		} else if(command.equals("/updateUser.user")) {	//나의 정보 수정하기
			String id=request.getParameter("id");
			String address=request.getParameter("address");
			String email=request.getParameter("email");
			...
			if(isS) {
				...
				jsForward("탈퇴 실패", "error.jsp?msg=탈퇴실패", response);
			}
	         ...
	         //java의 결과값(text)를 브라우저로 출력시키면 $.ajax()가 받는다.
